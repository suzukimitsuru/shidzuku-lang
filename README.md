...
`int8/16/32/64/128/256(s)`・`uint8/16/32/64/128/256(s)`・`float32/64` で、**幅が上限を、位取り(小数点以下の桁数)が刻みを決めます**。
単位の欄は必須で、単位なしは `none` です。`float128` はハードウェアと出力先の事情から置きません(ADR 0021)。
