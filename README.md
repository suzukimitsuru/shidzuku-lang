...
- データの型は10 の基本型と4つの構成子。**型の名前が値の出どころを告げる**(ADR 0009・0019・0020)。数値型は ADR 0021 として起案し直した(下記)
...
**2026-09-10 に ADR 0021(数値型の体系)として起案し直しました。** `十進数` は刻み(小数点以下の桁数)だけを宣言して上限を型から外し
(`十進数(i.f)` は廃止。単位付き・静的位取り・任意精度の十進固定小数点数であり、Haskell の `Data.Fixed` と同型)、
`整数` → `十進数` の昇格だけを暗黙にする案です。型に序列は置きません(ADR 0017 の「主に使う型」は引き継ぎません)。
併せて U14(プログラムの宣言)の名前空間案(逆ドメイン名)も撤回し、思考実験 15 で再検討しています。
思考実験 16〜18 で整数型の要否と他言語の数値型を調べ、**2026-09-16 に数値型を確定しました** —
`int8/16/32/64/128/256(s)`・`uint8/16/32/64/128/256(s)`・`float32/64` と、内部計算用の `dec(整数部.小数部)` の4種です。
**幅(`dec` は宣言した桁)が上限を、位取り(小数点以下の桁数)が刻みを決めます**。単位の欄は必須で、単位なしは `none` です。
`dec` は格納が値による可変長ですが上界は宣言が言い、データフロー・外部インターフェイス・状態には書けません。
`float128` はハードウェアと出力先の事情から置きません(ADR 0021)。
判断待ちは ADR 0006・0012・0018 と、数値型の 0021 です。どの順に採否を判断するかは第4節にあります。
