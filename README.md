...
- U5(トランスパイル先の言語) — ADR 0012 を起案しました(提案)。**動作環境は Rust の1実装**とし、トランスパイル先の言語で書くのは**ユニットだけ**とする案です。
  トランスパイル先は主要言語11種の比較を経て **Rust + TypeScript**(接合の距離がゼロの言語と最大の言語)、移植層はユニットと動作環境の**接合面(8つの呼び出し)**。採否をご判断ください。
  - 併せて U15(ユニットのバージョンと移行)を新設しました。別言語・別バージョンのユニットが同時に動く状態の扱いは、U5 の範囲を超えるため Phase 1 で決めます。
