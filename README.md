...
**2026-09-10 に ADR 0021(数値型の体系)として起案し直しました。** 主に使う型を `実数` にし、`十進数` は刻み(小数点以下の桁数)だけを宣言して
上限を型から外す案です(`十進数(i.f)` は廃止。単位付き・静的位取り・任意精度の十進固定小数点数であり、Haskell の `Data.Fixed` と同型)。
