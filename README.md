...
- データの型は10 の基本型と4つの構成子。**型の名前が値の出どころを告げる**(ADR 0009・0019・0020)。数値型は再検討中(下記)
...
思考実験 14 で比較し、7つの論点にまとめてあります(詳細は [docs/PLAN.md](docs/PLAN.md) 第7節)。
併せて U14(プログラムの宣言)の名前空間案(逆ドメイン名)も撤回し、思考実験 15 で再検討しています。
判断待ちは ADR 0006・0012・0018 と、数値型の暫定 ADR 0021 です。どの順に採否を判断するかは第4節にあります。
